get_recommended_videos <- function (video_id = NULL, max_results = 50, safe_search = "none", 
          ...) 
{
  ...
  resdf <- read.table(text = "", col.names = c("video_id", 
                                               "rel_video_id", "publishedAt", "channelId", "title", 
  ...
  if (length(res$items) != 0) {
    ...
    resdf <- cbind(video_id = video_id, rel_video_id = rel_video_id, 
...
tuber_GET <- function(path, query, ...) {
  ...
  req <- httr::GET("https://www.googleapis.com", path = paste0("youtube/v3/", path),
             query = query, httr::config(token = getOption("google_token")), ...)
